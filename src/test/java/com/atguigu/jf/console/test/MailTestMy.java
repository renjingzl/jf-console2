...
public class MailTestMy {
	...
	@Test
	public void testJavaMail() throws AddressException, MessagingException{
		...
		session.setDebug(true);	// 调试完毕后，注释掉
		
		
		//2、创建代表邮件内容的Message对象（JavaMail创建的邮件是基于MIME协议的）
		MimeMessage message = new MimeMessage(session);
		message.setFrom(new InternetAddress("769262847rj.@sina.com"));	// 发件人
		message.setSubject("这是本次报表"); // 设置主题
		message.setRecipient(RecipientType.TO, new InternetAddress("769262847@qq.com"));// 设置收件人
		// 设置正文
		// 纯文本的邮件
		message.setText("哈哈哈...");
	...
	@Test
	public void sendEmail2() throws AddressException, MessagingException{
		/**
		 *  1、通过session创建邮件的配置信息
			2、创建代表邮件内容的Message对象（JavaMail创建的邮件是基于MIME协议的）
			3、创建Transport对象、连接服务器、发送Message、关闭连接
